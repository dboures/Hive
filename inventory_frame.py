    ...
    def __init__(self, background, pos, state, white=True):
        ...
        for i in range(0, 5):
            ...
            if i == 0:
                ...
                tiles.append(Tile(tile_pos, 20, self.color,
                                  piece=Queen(self.color)))
            if i == 1:
                for j in [1, 3]:
                    tile_pos = tile_pos = (inner_left + (i * stock_width) + (
                        stock_width / 2), inner_top + title_height + (j * stock_height / 4))
                    tiles.append(Tile(tile_pos, 20, self.color,
            ...
            if i == 4:
                for j in [25, 67, 109]:
                    tile_pos = tile_pos = (inner_left + (i * stock_width) + (
                        stock_width / 2), inner_top + title_height + (j * stock_height / 135))
                    tiles.append(
                        Tile(tile_pos, 20, self.color, piece=Ant(self.color)))
        for tile in tiles:
